...
func main() {
	...
	if databaseURL == "" {
		databaseURL = "postgresql://postgres:postgres@localhost:5432/testdb?sslmode=disable"
		logger.Warn("DATABASE_URL not set, using default", "url", "postgresql://postgres:***@localhost:5432/testdb")
	}
	...
	// Connect to database using pgxpool (recommended for PostgreSQL)
	logger.Info("connecting to PostgreSQL database with pgxpool...")
...
func runExamples(ctx context.Context, db *kdbx.PostgresDB, logger *slog.Logger) error {
...
func exampleBasicQuery(ctx context.Context, db *kdbx.PostgresDB, logger *slog.Logger) error {
...
func exampleTransaction(ctx context.Context, db *kdbx.PostgresDB, logger *slog.Logger) error {
	...
	err = db.WithTransaction(ctx, func(tx kdbx.Tx) error {
		...
		userID1 := uuid.New()
		_, err := tx.Exec(ctx,
			"INSERT INTO temp_users (id, name, email) VALUES ($1, $2, $3)",
...
func exampleBatchOperations(ctx context.Context, db *kdbx.PostgresDB, logger *slog.Logger) error {
	...
	for _, p := range products {
		id := uuid.New()
		batch.Add(
			"INSERT INTO temp_products (id, name, price) VALUES ($1, $2, $3)",
			id, p.name, p.price,
		)
	}
...
func exampleHealthCheck(ctx context.Context, db *kdbx.PostgresDB, logger *slog.Logger) error {
...
func examplePoolStats(db *kdbx.PostgresDB, logger *slog.Logger) {
